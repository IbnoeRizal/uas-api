...
export const Enrollment_delete = z.object({
    id: z.number().nonnegative("id tidak boleh negatif").optional(),
    userId: z.number().nonnegative("id tidak boleh negatif").optional(),
    courseId: z.number().nonnegative("id tidak boleh negatif").optional(),
}).refine(
    ({id,userId,courseId})=>id || (userId && courseId), 
    {message:"parameter harus terdiri dari id enrollment atau userId dan courseId"}
);

export const Enrollment_update = z.object({
    userId: z.number().nonnegative("id tidak boleh negatif").optional(),
    courseId: z.number().nonnegative("id tidak boleh negatif").optional(),
}).refine(
    (data)=>Object.keys(data).length > 0,
    {message:"Minimal harus ada 1 field untuk diupdate"}
)
